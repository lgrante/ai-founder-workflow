...
8. **Étiquette git pour les sessions futures.** `/setup` tourne sur sa branche dédiée `setup-workflow`. Mais **toutes les sessions ultérieures** (spec, code, test, research, feedback, support, post, article, newsletter, report) suivront la convention de nommage de branche définie dans `templates/docs/WORKFLOW.md` § Étiquette git (préfixes par axe : `feat/`, `fix/`, `research/`, `feedback/`, `support/`, `post/`, `article/`, `newsletter/`, `report/`). Vérifie que le fichier WORKFLOW.md est copié en Phase 2 et mentionne cette convention dans le compte-rendu final de Phase Finale.
...
├── .mcp.json                 # serveur MCP Serena (code-graph) — navigation symbolique
├── .serena/project.yml       # langages indexés par défaut (typescript/python/php) ; cache/ gitignored
...
├── backlog/                  # pont Découverte→Build — items groomés par /backlog, lus par /spec
│   ├── <slug>.md             # un candidat à spécifier (statut idea/triaged/specced/dropped + priorité)
...
5. **Phase 3 — Outillage** : importe les 14 skills depuis `templates/.claude/skills/` (setup, spec, code, test côté dev ; research, feedback, support côté découverte ; backlog en pont Découverte→Build ; post, article, newsletter, report côté audience ; status + update transverses), les hooks (`test-gate.sh` + `preflight-guard.py` **obligatoire** comme garde-fou déterministe + `md-to-html.py` jumeau HTML + `backlog-lint.py` validateur d'items backlog + `register-hook.py` helper + 2 optionnels `context-handoff.sh` et `context-restore.sh`), `settings.json` (qui enregistre `preflight-guard.py` sur `UserPromptSubmit`, `test-gate.sh` sur `Stop`, `md-to-html.py` + `backlog-lint.py` sur `PostToolUse`), `statusline.sh`. Modifie `.gitignore` pour versionnement partiel `.claude/` (ignore `settings.local.json`, `scheduled_tasks.lock`, `worktrees/`) **et ajoute `knowledge/dashboard.html`** (vue *latest* de `/status`, locale — peut contenir des PII en mode privé ; cf. `docs/WORKFLOW.md` § Dashboard latest). **Pose le marqueur de version** : copie le `KIT_VERSION` du kit dans `.claude/.kit-version` (c'est ce que `/update` relira pour calculer le delta des prochaines mises à jour ; cf. `docs/WORKFLOW.md` § Propagation). **Important** : vérifie que `preflight-guard.py` est exécutable (`chmod +x`) et smoke-teste : `echo '{"prompt":"/spec foo"}' | .claude/hooks/preflight-guard.py` → si `docs/WORKFLOW.md` existe (donc workflow installé), exit silencieux ; sinon, JSON `decision:block`. **Stack anti-tokens (Économie de tokens, cf. `docs/WORKFLOW.md`)** :
   - **Serena (code-graph MCP)** : copie `templates/.mcp.json` → `.mcp.json` (serveur `serena`) et `templates/.serena/project.yml` → `.serena/project.yml` (langages par défaut `typescript`/`python`/`php`). Si `.mcp.json` existe déjà (autres serveurs), n'écrase pas : ajoute chirurgicalement le bloc `serena` (policy `suggest-only`). Smoke : `command -v serena` — absent → astuce `uv tool install serena-agent` (non bloquant, le MCP connectera quand le binaire sera là). **Attention** : n'ajoute un langage à `project.yml` que si son LSP est installable (un LSP manquant fait échouer l'init Serena pour tout le repo — ex. `go` exige `gopls`).
   - **chop (compresseur console)** : si `command -v chop` → lance `chop init` (per-repo, idempotent ; chop pose son propre hook). Absent → astuce `curl -fsSL https://raw.githubusercontent.com/AgusRdz/chop/main/install.sh | sh`. Ne réimplémente jamais le hook de chop à la main.
   **Note** : suggère à l'utilisateur d'installer aussi le skill compagnon `nano-banana` globalement s'il prévoit de générer des images de posts (cf. README § Compagnons optionnels). **Commit Phase 3.**
...
8. **Final** : **propose** le push de la branche `setup-workflow` (avec OK explicite séparé — aucun push automatique). Rappelle à l'utilisateur : merger sur `main` quand satisfait, vider `_archive/` après validation finale, supprimer le tag `pre-setup-<timestamp>` quand tout va bien. Suggère des dry-run pour valider les pipelines en bout-en-bout : `/backlog` (pont Découverte→Build), `/spec <feature>` (build), `/support <client>` (découverte si système de tickets configuré), `/post linkedin <sujet>` (audience), `/report linkedin` (analytics si MCP réseau configuré), `/status` (snapshot 360° + `knowledge/dashboard.html`, pour valider que tout est en place).
...
- L'outillage marche : `/hooks` montre le `Stop` hook chargé ; `/setup /spec /code /test /research /feedback /support /backlog /post /article /newsletter /report /status /update` apparaissent (les 14).
- Smoke-test du hook : `echo '{}' | .claude/hooks/test-gate.sh ; echo "exit=$?"` retourne `exit=0` (vert) ou un JSON `decision:block` parsable.
- **Stack anti-tokens en place** : `.mcp.json` (bloc `serena`) et `.serena/project.yml` (langages par défaut) présents ; `.serena/cache/` gitignored ; chop initialisé si le binaire est présent (sinon astuce affichée). Prérequis `serena`/`chop` manquants = dégradation propre, jamais bloquant.
...
- **Repo déjà setup** (`.claude/skills/spec/SKILL.md` existe) : signale-le. Pour une **mise à jour** (propager les améliorations du kit), **délègue à `/update`** — c'est son rôle (clone auto + propagation par catégorie selon `kit-manifest.json`, sans écraser les fichiers customisés ; cf. `docs/WORKFLOW.md` § Propagation). N'utilise `/setup` que pour une **ré-installation complète** (toutes les phases), et seulement sur demande explicite — jamais auto. **L'archéologie peut quand même tourner** sur les sessions passées (utile pour récupérer ce qui n'aurait pas été commité depuis le dernier setup).
