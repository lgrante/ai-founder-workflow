...
├── .claude/
│   ├── settings.json         # hooks (Stop, UserPromptSubmit, PostToolUse, SessionStart, PreCompact) + outputStyle: founder
│   ├── hooks/test-gate.sh    # filet rapide (anti-boucle stop_hook_active obligatoire)
│   ├── hooks/context-restore.sh # SessionStart — bandeau de reprise (branche/type/artefacts/état)
