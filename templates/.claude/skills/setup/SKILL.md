...
│   └── skills/               # 13 skills : setup, spec, code, test (dev) + research, feedback, support (découverte) + backlog (pont) + post, article, newsletter, report (audience) + status (transverse)
...
- L'outillage marche : `/hooks` montre le `Stop` hook chargé ; `/setup /spec /code /test /research /feedback /support /backlog /post /article /newsletter /report /status` apparaissent (les 13).
