...
---
Tu démarres l'installation interactive du workflow ai-dev-workflow sur le repo courant. Référence : `README.md` + `templates/docs/WORKFLOW.md` du repo kit, doctrine identique à `DEPLOY.md`.
...
5. **Phase 3 — Outillage** : importe les 5 skills `/spec /code /test /research /feedback` depuis `templates/.claude/skills/`, les hooks (`test-gate.sh` + 2 optionnels), `settings.json`, `statusline.sh`. Modifie `.gitignore` pour versionnement partiel `.claude/` (ignore `settings.local.json`, `scheduled_tasks.lock`, `worktrees/`). **Commit Phase 3.**
