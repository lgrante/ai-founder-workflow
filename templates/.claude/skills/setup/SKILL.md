...
│   └── skills/               # 6 skills : setup, spec, code, test, research, feedback
...
│   └── crm/contacts/         # données perso — gitignored OU repo séparé
...
Détail complet : `templates/docs/WORKFLOW.md` du kit (Convention par-feature + Discovery).
...
2. **Phase 0 — Inventaire & plan (lecture seule)** :
   - Inventorie : arborescence, code applicatif (stack), `.claude/` existant, `CLAUDE.md`, hooks/skills présents, **setup de test** (frameworks unit/e2e, commandes exactes), sous-repos / submodules, axe par-rôle résiduel, entanglements code↔orchestration.
   ...
   - Lance `AskUserQuestion` pour les décisions stratégiques : périmètre (code applicatif KEEP ?), sort des dossiers par-rôle, setup de test, CRM (versionné ici ou repo séparé ?), préfixes de session, sous-repos formalisés en submodules ou pas, worktrees morts à supprimer ?
   - **Attends la validation complète (plan + carte + questions) AVANT toute action.**

3. **Phase 1 — Restructuration** : annonce le batch de `git mv`. Crée l'ossature (`mkdir -p _archive/`, `knowledge/`, `features/`, `.cc-scratch/`, `.claude/skills/`, `docs/`). Exécute les `git mv` validés. Archive READMEs obsolètes. Supprime worktrees morts (autorisation explicite). **Commit Phase 1.**

4. **Phase 2 — Doctrine** : crée `CLAUDE.md` (court, rempli avec specs du repo), copie `docs/WORKFLOW.md` depuis `templates/docs/WORKFLOW.md` du kit, crée READMEs racine + `knowledge/` + `features/`, crée `knowledge/insights.md` squelette. **Commit Phase 2.**

5. **Phase 3 — Outillage** : importe les skills depuis `templates/.claude/skills/` (spec, code, test, research, feedback ; ainsi que setup pour cohérence), les hooks (`test-gate.sh` + 2 optionnels `context-handoff.sh` et `context-restore.sh`), `settings.json`, `statusline.sh`. Modifie `.gitignore` pour versionnement partiel `.claude/` (ignore `settings.local.json`, `scheduled_tasks.lock`, `worktrees/`). **Commit Phase 3.**
...
8. **Final** : propose le push de la branche `setup-workflow`. Rappelle à l'utilisateur : merger sur `main` quand satisfait, vider `_archive/` après validation finale. Suggère un dry-run avec `/spec <feature>` sur une mini-feature pour valider le pipeline en bout-en-bout.
...
- L'outillage marche : `/hooks` montre le `Stop` hook chargé ; `/setup /spec /code /test /research /feedback` apparaissent.
- Smoke-test du hook : `echo '{}' | .claude/hooks/test-gate.sh ; echo "exit=$?"` retourne `exit=0` (vert) ou un JSON `decision:block` parsable.
- Dry-run d'une petite feature passe le pipeline de bout en bout : `/spec` → `SPEC.md` → `/code` → étape → hook vert → `/test` au jalon → gate humain.

## Cas limites

- **Repo vierge** (pas de `.claude/`, pas de `features/`) : Phase 0 minimale (rien à migrer), scaffold direct la structure standard.
...
<!-- Exemple d'usage :
  ...
  → Phase 0 : inventorie 247 fichiers, propose carte + AskUserQuestion (4 questions)
  → utilisateur valide → Phase 1 : « je vais faire 12 git mv pour structurer features/ et knowledge/ — OK ? »
  → OK → exécute → commit Phase 1
  → Phase 2 : « je vais créer CLAUDE.md + docs/WORKFLOW.md + 4 READMEs — OK ? » → ...
